...
var wheel = {
  ...
  spin: function (serverWheel) {
    // if only Admin


    // Start the wheel only if it's not already spinning
    wheel.angleCurrent = serverWheel.currentAngle

    if (wheel.timerHandle == 0) {
      wheel.spinStart = serverWheel.spinStart;
      wheel.maxSpeed = serverWheel.maxSpeed; // Randomly vary how hard the spin is
      wheel.frames = 0;
  ...
  update: function (randomNumber) {
    // Ensure we start mid way on a item
    randomNumber = randomNumber ? randomNumber : Math.random();
    var r = Math.floor(randomNumber * wheel.segments.length);
    //var r = 0;
